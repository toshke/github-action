...
function handleInput(input, cb, userOptions) {
  ...
  index.getOptions((err, options) => {
    ...
    if (input.format == 'coveralls') {
        console.log(`Using coveralls format, using source_files element from file provided`);
        console.log(`Options: `, JSON.stringify(options));
        const providedCoveralls = JSON.parse(input.file);
        const postData = Object.assign({}, options, {source_files: providedCoveralls.source_files});
        index.sendToCoveralls(postData, (err, response, body) => {
